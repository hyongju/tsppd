...
h1 = figure('position',[100 100 800 800],'Color',[1 1 1]);
plot(vert(:,1),vert(:,2),'ok','MarkerSize',10,'LineWidth',2); hold on;
hold on;
plot(vert(1:k,1),vert(1:k,2),'ob','MarkerSize',14,'LineWidth',2); hold on;
...
set(gca,'FontSize',16);
title(sprintf('# of customers: %d, # of vehicles: %d, capacity: %d',n,k,q));
set(findall(h1, 'Type', 'Text'),'FontWeight', 'Normal')
% 
% plot constraint#1
for i = 1:k
    ...
    plot([0,bldLst],[0;rst(bldLst,:)],'bo-','MarkerSize',10,'LineWidth',2);
    ...
    title(sprintf('vehicle #%d, capacity: %d',i,q))
    set(findall(h2, 'Type', 'Text'),'FontWeight', 'Normal')
    
end
% plot constraint#2
clear rst
h3 = figure('position',[100 100 800 600],'Color',[1 1 1]);
for l = 1:n
    eini = zeros(1,v);    
    eini(l+k) = 1;
    eini(l+n+k) = -1;        
...
set(gca,'FontSize',16);
xlabel('customer ID');
ylabel('precedence (must be non-positive)')
set(findall(h3, 'Type', 'Text'),'FontWeight', 'Normal')
...
set(gca,'FontSize',16);
set(findall(h4, 'Type', 'Text'),'FontWeight', 'Normal')
